...
def clean_channel_link(channel_link: str) -> str:
    """
    Clean and normalize a Telegram channel link.
    
    Handles:
    - URLs: https://t.me/channelname -> @channelname
    - Usernames: channelname -> @channelname
    - Numeric IDs: -1001234567890 -> -1001234567890 (no @ added)
    
    Args:
        channel_link: Raw channel link input
        
    Returns:
        Cleaned channel link
    """
    ...
    # Check if it's a numeric ID (starts with - and followed by digits)
    # Channel IDs are typically like -1001234567890
    elif channel_link.startswith('-') and channel_link.lstrip('-').isdigit():
        # Numeric ID - keep as is (don't add @)
        pass
    # Check if it's already a username with @
    elif channel_link.startswith('@'):
        # Already has @, keep as is
        pass
    else:
        # Username without @ - add @
